...
export default function LandingPageHero({ onStart }: LandingPageProps) {
  ...
  const handleStart = () => {
    ...
    // Aggressive video cleanup
    const stopVideo = (ref: React.RefObject<HTMLVideoElement | null>) => {
        if (ref.current) {
            ref.current.pause();
            ref.current.removeAttribute('src'); // Help GC
            ref.current.load();
        }
    };
    
    stopVideo(videoRef);
    stopVideo(previewVideoRef);

    // Allow UI to update but keep it snappy
    setTimeout(() => {
      onStart();
    }, 150);
  };
      ...
      <section className="relative pt-8 pb-16 px-6 overflow-hidden max-w-md mx-auto md:max-w-4xl">
        ...
        <div className="relative z-10 flex flex-col items-center max-w-4xl mx-auto pt-8">
          ...
          <motion.div
            ...
            className="flex flex-col items-center gap-1 mt-4 mb-4 relative z-20"
          >
