...
export default function PageContainer({ children, className = "" }: { children: ReactNode, className?: string }) {
  return (
    <div className={twMerge(clsx("w-full min-h-screen md:min-h-[297mm] print:h-auto print:min-h-0 relative bg-[#FAF9F7] text-[#2C2F4A] overflow-hidden print:overflow-visible flex flex-col", className))}>
      {children}
