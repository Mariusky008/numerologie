...
export default function Part6Integration({ userData, results }: { userData: UserData, results: NumerologyResult }) {
  return (
    <>
      ...
      {[1, 2].map((page) => (
        <PageContainer key={page} className="p-4 md:p-16 bg-[#fffbf0] text-[#57534e]">
          <h2 className="text-2xl md:text-4xl font-serif text-[#78350f] mb-8 md:mb-12 border-b-2 border-[#d97706] pb-4 inline-block">
          ...
          <div className="space-y-8">
            {[1, 2].map((weekOffset) => {
              ...
              return (
                <div key={week} className="flex flex-col md:flex-row gap-4 md:gap-8 border-b border-[#d97706]/20 pb-8">
                  <div className="text-4xl md:text-6xl font-serif text-[#d97706] opacity-30">0{week}</div>
                  <div>
                    <h4 className="text-lg md:text-2xl font-bold mb-4 text-[#78350f]">Semaine {week} : {
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-2">
                      {[1, 2, 3, 4, 5, 6, 7].map(d => (
                        <div key={d} className="w-8 h-8 rounded-full border border-[#d97706] flex items-center justify-center text-xs text-[#d97706] flex-shrink-0 bg-white">
                          J{d}
                        </div>
                      ))}
                    </div>
